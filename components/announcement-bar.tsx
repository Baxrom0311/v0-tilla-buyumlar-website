...
const items = [
  { icon: Truck, text: "5 000 000 so'mdan yuqori xaridlarga bepul yetkazib berish" },
...
export function AnnouncementBar() {
  return (
    <div className="bg-ink text-cream overflow-hidden border-b border-gold/20">
      <div className="flex items-center gap-12 py-2.5 whitespace-nowrap animate-marquee">
        {[...items, ...items, ...items].map((item, i) => {
          const Icon = item.icon
          return (
            <div key={i} className="flex items-center gap-2 text-xs tracking-wide">
